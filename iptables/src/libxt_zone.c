...
static unsigned int
parse_zone_names(const char *zonestring, struct ipt_zone_info_v1 *info,
		 size_t max_length)
{
	char *buffer, *cp, *next;
	unsigned int i;
	...
	for (cp = buffer, i = 0; cp && i < IPT_ZONE_NAME_COUNT;
	     cp = next, ++i) {
		next = strchr(cp, ',');
		if (next)
			*next++ = '\0';
...
{
	struct ipt_zone_info *info =
	    (struct ipt_zone_info *) (*match)->data;
	      ...
	      const void *entry, struct xt_entry_match **match)
{
	struct ipt_zone_info_v1 *info =
	    (struct ipt_zone_info_v1 *) (*match)->data;
...
static void zone_final_check(unsigned int flags)
{
...
static void
zone_print_v0(const void *ip, const struct xt_entry_match *match,
	      int numeric)
{
...
static void
zone_print_v1(const void *ip, const struct xt_entry_match *match,
	      int numeric)
{
	struct ipt_zone_info_v1 *info =
	    (struct ipt_zone_info_v1 *) match->data;
