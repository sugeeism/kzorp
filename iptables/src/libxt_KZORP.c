...
static void parse_kzorp_mark(char *s, struct xt_kzorp_target_info *info)
{
	...
	if (!xtables_strtoui(s, &end, &value, 0, UINT32_MAX))
		xtables_param_act(XTF_BAD_VALUE, "KZORP", "--tproxy-mark",
				  s);
	if (*end == '/')
		if (!xtables_strtoui(end + 1, &end, &mask, 0, UINT32_MAX))
			xtables_param_act(XTF_BAD_VALUE, "KZORP",
					  "--tproxy-mark", s);
	if (*end != '\0')
		xtables_param_act(XTF_BAD_VALUE, "KZORP", "--tproxy-mark",
				  s);
...
static int kzorp_tg_parse(int c, char **argv, int invert,
			  ...
			  struct xt_entry_target **target)
{
	...
	switch (c) {
	case '1':
		xtables_param_act(XTF_ONLY_ONCE, "KZORP", "--tproxy-mark",
				  *flags & PARAM_MARK);
		xtables_param_act(XTF_NO_INVERT, "KZORP", "--tproxy-mark",
				  invert);
...
static void kzorp_tg_print(const void *ip,
			   const struct xt_entry_target *target,
			   int numeric)
{
	const struct xt_kzorp_target_info *info =
	    (const void *) target->data;
	printf("KZORP mark 0x%x/0x%x", (unsigned int) info->mark_value,
	       (unsigned int) info->mark_mask);
}

static void kzorp_tg_save(const void *ip,
			  const struct xt_entry_target *target)
{
	const struct xt_kzorp_target_info *info =
	    (const void *) target->data;
