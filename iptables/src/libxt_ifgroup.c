...
static void ifgroup_help(void)
{
	printf("ifgroup v%s options:\n"
...
static void ifgroup_final_check(unsigned int flags)
{
	if (!flags)
		xtables_error(PARAMETER_PROBLEM,
...
static void ifgroup_print_value_in(struct xt_ifgroup_info *info)
{
...
static void ifgroup_print_value_out(struct xt_ifgroup_info *info)
{
