...
static void __init transaction_init(void)
{
...
inline static struct kz_transaction *transaction_lookup(int peer_pid)
{
...
/* !!! must be called with the transaction mutex held !!! */
static struct kz_transaction *transaction_create(const netlink_port_t
						 peer_pid,
						 const unsigned int
						 instance_id,
						 u_int64_t config_cookie)
{
...
/* !!! must be called with the transaction mutex held !!! */
static void transaction_destroy(struct kz_transaction *t)
{
...
transaction_add_op(struct kz_transaction *tr,
		   ...
		   void *const data, void (*cleanup_func) (void *))
{
...
static void transaction_destroy_zone(void *data)
{
	kz_zone_put((struct kz_zone *) data);
}

static void transaction_destroy_service(void *data)
{
	kz_service_put((struct kz_service *) data);
}

static void transaction_destroy_dispatcher(void *data)
{
	kz_dispatcher_put((struct kz_dispatcher *) data);
}

static void transaction_destroy_bind(void *data)
{
	kz_bind_destroy((struct kz_bind *) data);
}

/* caller must mutex the passed transaction! */
static void transaction_cleanup_op(struct kz_transaction *tr)
{
...
/* caller must mutex the passed transaction! */
static struct kz_zone *transaction_zone_lookup(const struct kz_transaction
					       *const tr, const char *name)
{
...
/* caller must mutex the passed transaction! */
static struct kz_service *transaction_service_lookup(const struct
						     kz_transaction *const
						     tr, const char *name)
{
	const struct kz_operation *i;
...
/* caller must mutex the passed transaction! */
static struct kz_dispatcher *transaction_dispatcher_lookup(const struct
							   kz_transaction
							   *const tr,
							   const char
							   *name)
...
static struct kz_bind *transaction_bind_lookup(const struct kz_transaction
					       *const tr,
 ...
 */
static struct kz_dispatcher_n_dimension_rule *transaction_rule_lookup(const
								      struct
								      kz_transaction
								      *const
								      tr,
 ...
 */
static inline struct kz_zone *lookup_zone_merged(const struct
						 kz_transaction *const tr,
						 const char *name)
{
 ...
 * removed by a subsequent commit.
 **/
static inline struct kz_service *lookup_service_merged(const struct
						       kz_transaction
						       *const tr,
						       const char *name)
{
...
kznl_parse_name(const struct nlattr *attr, char *name, unsigned long nsize)
{
	...
	if (nsize < length + 1) {
		kz_err
		    ("invalid target length; dst_size='%lu', len='%lu'\n",
		     nsize, length);
		return -EINVAL;
	}
...
static int kznl_parse_name_alloc(const struct nlattr *attr, char **name)
{
	struct kza_name *a = nla_data(attr);
	char *n;
	int res;
...
static const struct nla_policy inet_addr_nla_policy[KZNL_ATTR_TYPE_COUNT +
						    1] = {
...
static inline int
kznl_parse_inet_addr(const struct nlattr *attr, union nf_inet_addr *addr,
		     sa_family_t * family)
{
	int res = 0;
	...
	res =
	    nla_parse_nested(tb, KZNL_ATTR_TYPE_COUNT, attr,
			     inet_addr_nla_policy);
	if (res < 0) {
		kz_err("failed to parse nested attribute\n");
		return res;
	}

	kz_debug("nested attributes: %p %p", tb[KZNL_ATTR_INET_ADDR],
		 tb[KZNL_ATTR_INET6_ADDR]);
	if (tb[KZNL_ATTR_INET_ADDR]) {
		res =
		    kznl_parse_in_addr(tb[KZNL_ATTR_INET_ADDR], &addr->in);
		if (res < 0) {
		...
		res =
		    kznl_parse_in6_addr(tb[KZNL_ATTR_INET6_ADDR],
					&addr->in6);
		if (res < 0) {
...
static inline int
kznl_parse_in_subnet(const struct nlattr *attr,
...
static inline int
kznl_parse_in6_subnet(const struct nlattr *attr, struct in6_addr *addr,
		      struct in6_addr *mask)
{
	...
	ipv6_addr_set(&pfx, 0xffffffff, 0xffffffff, 0xffffffff,
		      0xffffffff);
...
static const struct nla_policy inet_subnet_nla_policy[KZNL_ATTR_TYPE_COUNT
						      + 1] = {
...
static inline int
kznl_parse_inet_subnet(const struct nlattr *attr, union nf_inet_addr *addr,
		       union nf_inet_addr *mask, sa_family_t * family)
{
	int res = 0;
	struct nlattr *tb[KZNL_ATTR_TYPE_COUNT + 1];
	...
	if (tb[KZNL_ATTR_INET_SUBNET]) {
		res =
		    kznl_parse_in_subnet(tb[KZNL_ATTR_INET_SUBNET],
					 &addr->in, &mask->in);
		if (res < 0) {
		...
		res =
		    kznl_parse_in6_subnet(tb[KZNL_ATTR_INET6_SUBNET],
					  &addr->in6, &mask->in6);
		if (res < 0) {
...
static inline int kznl_parse_port(const struct nlattr *attr, __u16 * _port)
{
	__u16 port;
...
static inline int
kznl_parse_port_range(const struct nlattr *attr, u_int16_t * range_from,
		      u_int16_t * range_to)
{
...
static inline int
kznl_parse_service_params(const struct nlattr *attr,
			  struct kz_service *svc)
{
	struct kza_service_params *a = nla_data(attr);
	u_int32_t new_flags = ntohl(a->flags);

	if (a->type <= KZ_SERVICE_INVALID
	    || a->type >= KZ_SERVICE_TYPE_COUNT)
		return -EINVAL;
	if ((new_flags | KZF_SERVICE_PUBLIC_FLAGS) !=
	    KZF_SERVICE_PUBLIC_FLAGS)
		return -EINVAL;
...
kznl_parse_service_router_dst(struct nlattr *cda[], struct kz_service *svc)
{
	...
	res =
	    kznl_parse_inet_addr(cda[KZNL_ATTR_SERVICE_ROUTER_DST_ADDR],
				 &svc->a.fwd.router_dst_addr,
	...
	res =
	    kznl_parse_port(cda[KZNL_ATTR_SERVICE_ROUTER_DST_PORT],
			    &svc->a.fwd.router_dst_port);
	if (res < 0) {
...
static inline int
kznl_parse_service_nat_params(const struct nlattr *attr,
			      struct nf_nat_range *range)
{
	const struct kza_service_nat_params *a = nla_data(attr);
	u_int32_t flags = ntohl(a->flags);

	if ((flags | KZF_SERVICE_NAT_MAP_PUBLIC_FLAGS) !=
	    KZF_SERVICE_NAT_MAP_PUBLIC_FLAGS)
		return -EINVAL;
...
static inline int
kznl_parse_service_session_cnt(const struct nlattr *attr,
			       u_int32_t * count)
{
...
static inline int
kznl_parse_service_deny_method(const struct nlattr *attr,
			       unsigned int *type)
{
...
static inline int
kznl_parse_service_ipv4_deny_method(const struct nlattr *attr,
				    unsigned int *_type)
{
	unsigned int type;
	int res;
...
static inline int
kznl_parse_service_ipv6_deny_method(const struct nlattr *attr,
				    unsigned int *_type)
{
	unsigned int type;
	int res;
...
static int
kznl_parse_dispatcher_n_dimension(const struct nlattr *attr,
				  struct kz_dispatcher *n_dimension)
{
	struct kza_dispatcher_n_dimension_params *a = nla_data(attr);

	return kz_dispatcher_alloc_rule_array(n_dimension,
					      ntohl(a->num_rules));
}
...
kznl_parse_dispatcher_n_dimension_rule_entry(const struct nlattr *attr,
					     ...
					     *rule_entry)
{
	struct kz_dispatcher_n_dimension_rule_entry_params *a =
	    nla_data(attr);
...
static inline int
kznl_parse_get_version_params(const struct nlattr *attr,
		      ...
		      const union nf_inet_addr *mask)
{
	...
	if (family == AF_INET) {
		kz_debug("dump inet subnet; address='%pI4', mask='%pI4'\n",
			 &addr->in, &mask->in);
		res =
		    kznl_dump_in_subnet(skb, KZNL_ATTR_INET_SUBNET,
					&addr->in, &mask->in);
		if (res < 0)
		...
		kz_debug("dump inet subnet; address='%pI6', mask='%pI6'\n",
			 &addr->in6, &mask->in6);
		res =
		    kznl_dump_in6_subnet(skb, KZNL_ATTR_INET6_SUBNET,
					 &addr->in6, &mask->in6);
		if (res < 0)
			goto nla_put_failure;
	} else {
		    ...
		    sa_family_t family, const union nf_inet_addr *addr)
{
	...
	if (family == AF_INET) {
		...
		res =
		    nla_put(skb, KZNL_ATTR_INET_ADDR,
			    sizeof(struct in_addr), &addr->in);
		if (res < 0)
			goto nla_put_failure;
	} else if (family == AF_INET6) {
		kz_debug("dump inet addr; address='%pI6'\n", &addr->in6);
		res =
		    nla_put(skb, KZNL_ATTR_INET6_ADDR,
			    sizeof(struct in6_addr), &addr->in6);
		if (res < 0)
			goto nla_put_failure;
	} else {
		BUG();
...
static inline int
kznl_dump_service_nat_entry(struct kza_service_nat_params *a,
			    struct nf_nat_range *range)
{
...
static int kznl_recv_start(struct sk_buff *skb, struct genl_info *info)
{
	int res = 0;
	...
	/* parse attributes */
	if (kznl_parse_name_alloc
	    (info->attrs[KZNL_ATTR_INSTANCE_NAME], &ins_name) < 0) {
	...
	if (info->attrs[KZNL_ATTR_CONFIG_COOKIE]) {
		config_cookie =
		    be64_to_cpu(nla_get_u64
				(info->attrs[KZNL_ATTR_CONFIG_COOKIE]));
	}
	...
	if (ins == NULL) {
		ins =
		    kz_instance_create(ins_name, strlen(ins_name),
				       info->snd_pid);
		if (ins == NULL) {
...
*/
static int
kznl_recv_commit_transaction(struct kz_instance *instance,
			     struct kz_transaction *tr)
{
		...
		list_for_each_entry(io, &tr->op, list) {
			if (io->type == KZNL_OP_DISPATCHER) {
				const struct kz_dispatcher *dispatcher =
				    (struct kz_dispatcher *) io->data;

				if (dispatcher->num_rule !=
				    dispatcher->alloc_rule) {
					kz_err
					    ("rule number mismatch; dispatcher='%s', alloc_rules='%u', num_rules='%u'\n",
					     dispatcher->name,
					     dispatcher->alloc_rule,
					     dispatcher->num_rule);
					return -EINVAL;
	...
	/* process services */
	{
		...
		list_for_each_entry_safe(io, po, &tr->op, list) {
			if (io->type == KZNL_OP_SERVICE) {
				...
				list_del(&io->list);
				list_add_tail(&svc->list,
					      &new->services.head);
				kfree(io);
				kz_debug("add service; name='%s'\n",
					 svc->name);
				orig =
				    kz_service_lookup_name(old, svc->name);
				if (orig != NULL) {
					kz_debug
					    ("migrate service session count\n");
					atomic_set(&svc->session_cnt,
						   kz_service_lock(orig));
	...
	/* process zones */
	{
		...
		if (!(tr->flags & KZF_TRANSACTION_FLUSH_ZONES)) {
			list_for_each_entry(i, &old->zones.head, list) {
				...
				if (zone == NULL)
					goto mem_error;
				kz_debug
				    ("clone zone; name='%s', depth='%u'\n",
				     zone->unique_name, zone->depth);
				list_add_tail(&zone->list,
					      &new->zones.head);
			}
		}
		...
		list_for_each_entry(i, &new->zones.head, list) {
			if (i->admin_parent != NULL) {
				...
				parent =
				    __kz_zone_lookup_name(&new->zones.head,
							  i->admin_parent->
							  unique_name);
				if (parent == NULL) {
					 ...
					 * internal error */
					kz_err
					    ("transaction problem: internal error, aborting\n");
					res = -EINVAL;
					goto error;
				}
				...
				kz_debug
				    ("set admin-parent for zone; name='%s' parent='%s', depth='%u', parent_depth='%u'\n",
				     i->unique_name, parent->unique_name,
				     i->depth, parent->depth);
			}
		}
	}
	/* process dispatchers */
	{
		...
		list_for_each_entry(i, &old->dispatchers.head, list) {
			...
			kz_debug
			    ("cloning dispatcher; name='%s', alloc_rules='%u'\n",
			     i->name, i->alloc_rule);
		...
		list_for_each_entry_safe(io, po, &tr->op, list) {
			if (io->type == KZNL_OP_DISPATCHER) {
				struct kz_dispatcher *dispatcher =
				    (struct kz_dispatcher *) io->data;
				kz_debug
				    ("add dispatcher; name='%s', alloc_rules='%u', num_rules='%u'\n",
				     dispatcher->name,
				     dispatcher->alloc_rule,
				     dispatcher->num_rule);
				list_del(&io->list);
				list_add_tail(&dispatcher->list,
					      &new->dispatchers.head);
				kfree(io);
			}
		}
		...
		list_for_each_entry(i, &new->dispatchers.head, list) {
			kz_dispatcher_relink(i, &new->zones.head,
					     &new->services.head);
		}
	}
	...
	if (res < 0) {
		kz_err
		    ("failed to build zone lookup data structures, aborting\n");
		goto error;
	}
...
static int kznl_recv_commit(struct sk_buff *skb, struct genl_info *info)
{
	int res = 0;
	struct kz_transaction *tr;
...
static int
kznl_recv_setflag(struct sk_buff *skb, struct genl_info *info,
		  unsigned int flag)
{
	int res = 0;
	struct kz_transaction *tr;
...
static int kznl_recv_flush_z(struct sk_buff *skb, struct genl_info *info)
{
	return kznl_recv_setflag(skb, info, KZF_TRANSACTION_FLUSH_ZONES);
}

static int kznl_recv_flush_s(struct sk_buff *skb, struct genl_info *info)
{
	return kznl_recv_setflag(skb, info,
				 KZF_TRANSACTION_FLUSH_SERVICES);
}

static int kznl_recv_flush_d(struct sk_buff *skb, struct genl_info *info)
{
	return kznl_recv_setflag(skb, info,
				 KZF_TRANSACTION_FLUSH_DISPATCHERS);
}

static int kznl_recv_flush_b(struct sk_buff *skb, struct genl_info *info)
{
	return kznl_recv_setflag(skb, info, KZF_TRANSACTION_FLUSH_BIND);
}

static int kznl_recv_add_zone(struct sk_buff *skb, struct genl_info *info)
{
	int res = 0;
	...
	/* fill fields */
	res =
	    kznl_parse_name_alloc(info->attrs[KZNL_ATTR_ZONE_NAME],
				  &zone->name);
	if (res < 0) {
	...
	if (info->attrs[KZNL_ATTR_ZONE_RANGE]) {
		res =
		    kznl_parse_inet_subnet(info->
					   attrs[KZNL_ATTR_ZONE_RANGE],
					   &zone->addr, &zone->mask,
					   &zone->family);
		if (res < 0) {
	...
	if (info->attrs[KZNL_ATTR_ZONE_UNAME]) {
		res =
		    kznl_parse_name_alloc(info->
					  attrs[KZNL_ATTR_ZONE_UNAME],
					  &zone->unique_name);
		if (res < 0) {
	...
	if (p != NULL) {
		kz_err
		    ("zone with the same unique name already present; name='%s'\n",
		     zone->unique_name);
		res = -EEXIST;
	...
	res =
	    transaction_add_op(tr, KZNL_OP_ZONE, kz_zone_get(zone),
			       transaction_destroy_zone);
	if (res < 0) {
...
static int
kznl_build_zone_add(struct sk_buff *skb, netlink_port_t pid, u_int32_t seq,
		    ...
		    const struct kz_zone *zone)
{
	...
	if (zone->flags & KZF_ZONE_HAS_RANGE) {
		if (kznl_dump_inet_subnet
		    (skb, KZNL_ATTR_ZONE_RANGE, zone->family, &zone->addr,
		     &zone->mask) < 0)
			goto nla_put_failure;
	}

	if (kznl_dump_name(skb, KZNL_ATTR_ZONE_UNAME, zone->unique_name) <
	    0)
		goto nla_put_failure;
	...
	if (zone->admin_parent != NULL) {
		if (kznl_dump_name
		    (skb, KZNL_ATTR_ZONE_PNAME,
		     zone->admin_parent->name) < 0)
			goto nla_put_failure;
	}
...
static int
kznl_build_zone(struct sk_buff *skb, netlink_port_t pid, u_int32_t seq,
		int flags, const struct kz_zone *zone,
		const struct kz_config *cfg)
{
	/* *part_idx and *entry_idx is left pointing the failed item */
	return kznl_build_zone_add(skb, pid, seq, flags, KZNL_MSG_ADD_ZONE,
				   zone);
}
...
static int kznl_recv_get_zone(struct sk_buff *skb, struct genl_info *info)
{
	int res = 0;
	char *zone_name = NULL;
	struct kz_zone *zone;
	...
	res =
	    kznl_parse_name_alloc(info->attrs[KZNL_ATTR_ZONE_UNAME],
				  &zone_name);
	if (res < 0) {
		kz_err("failed to parse zone name\n");
		goto error;
...
kznl_recv_add_service(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (!info->attrs[KZNL_ATTR_SERVICE_PARAMS]
	    || !info->attrs[KZNL_ATTR_SERVICE_NAME]) {
		kz_err("required attributes missing\n");
		res = -EINVAL;
		goto error;
	}
	...
	/* fill fields */
	res =
	    kznl_parse_service_params(info->
				      attrs[KZNL_ATTR_SERVICE_PARAMS],
				      svc);
	if (res < 0
	    || (svc->type != KZ_SERVICE_PROXY
		&& svc->type != KZ_SERVICE_FORWARD
		&& svc->type != KZ_SERVICE_DENY)) {
	...
	if (svc->type == KZ_SERVICE_FORWARD
	    && !(svc->flags & KZF_SERVICE_TRANSPARENT)) {
		if (!info->attrs[KZNL_ATTR_SERVICE_ROUTER_DST_PORT]) {
			kz_err
			    ("required router destination port attribute missing\n");
			res = -EINVAL;
			goto error_put_svc;
		}
		if (!info->attrs[KZNL_ATTR_SERVICE_ROUTER_DST_ADDR]) {
			kz_err
			    ("required router destination address attribute missing\n");
			res = -EINVAL;
			goto error_put_svc;
		}
	}
	...
	if (svc->type == KZ_SERVICE_DENY) {
		if (!info->attrs[KZNL_ATTR_SERVICE_DENY_IPV4_METHOD]) {
			kz_err
			    ("required IPv4 reject method attribute missing\n");
			res = -EINVAL;
			goto error_put_svc;
		}
		if (!info->attrs[KZNL_ATTR_SERVICE_DENY_IPV6_METHOD]) {
			kz_err
			    ("required IPv6 reject method attribute missing\n");
			res = -EINVAL;
			goto error_put_svc;
		}
	}
	...
	res =
	    kznl_parse_name_alloc(info->attrs[KZNL_ATTR_SERVICE_NAME],
				  &svc->name);
	if (res < 0) {
	...
	if (info->attrs[KZNL_ATTR_SERVICE_SESSION_CNT]) {
		res =
		    kznl_parse_service_session_cnt(info->
						   attrs
	...
	case KZ_SERVICE_FORWARD:
		...
		if (!(svc->flags & KZF_SERVICE_TRANSPARENT)) {
			res =
			    kznl_parse_service_router_dst(info->attrs,
							  svc);
			if (res < 0) {
				kz_err
	...
	case KZ_SERVICE_DENY:
		res =
		    kznl_parse_service_ipv4_deny_method(info->
							attrs
		...
		if (res < 0) {
			kz_err
			    ("failed to parse deny service IPv4 reject method\n");
			goto error_put_svc;
		}

		res =
		    kznl_parse_service_ipv6_deny_method(info->
							attrs
		...
		if (res < 0) {
			kz_err
			    ("failed to parse deny service IPv6 reject method\n");
			goto error_put_svc;
		}
	...
	if (p != NULL) {
		kz_err
		    ("service with the same name already present; name='%s'\n",
		     svc->name);
		res = -EEXIST;
		goto error_unlock_op;
	}
	...
	res =
	    transaction_add_op(tr, KZNL_OP_SERVICE, kz_service_get(svc),
			       transaction_destroy_service);
	if (res < 0) {
		kz_err("failed to queue transaction operation\n");
	}
...
static int
kznl_recv_add_service_nat(struct sk_buff *skb, struct genl_info *info,
			  bool snat)
{
	int res = 0;
	struct kz_service *svc;
	...
	if (!info->attrs[KZNL_ATTR_SERVICE_NAME]
	    || !info->attrs[KZNL_ATTR_SERVICE_NAT_SRC]
	...
	res =
	    kznl_parse_service_nat_params(info->
					  attrs[KZNL_ATTR_SERVICE_NAT_SRC],
					  &src);
	if (res < 0) {
	...
	res =
	    kznl_parse_service_nat_params(info->
					  attrs[KZNL_ATTR_SERVICE_NAT_MAP],
					  &map);
	if (res < 0) {
...
static int
kznl_build_service_add(struct sk_buff *skb, netlink_port_t pid,
		       ...
		       const struct kz_service *svc)
{
	...
	case KZ_SERVICE_FORWARD:
		if (!(svc->flags & KZF_SERVICE_TRANSPARENT)) {
			if (kznl_dump_inet_addr
			    (skb, KZNL_ATTR_SERVICE_ROUTER_DST_ADDR,
			     svc->a.fwd.router_dst_addr_family,
			     &svc->a.fwd.router_dst_addr) < 0)
				goto nla_put_failure;
			if (kznl_dump_port
			    (skb, KZNL_ATTR_SERVICE_ROUTER_DST_PORT,
	...
	case KZ_SERVICE_DENY:
		if (kznl_dump_service_deny_method
		    (skb, KZNL_ATTR_SERVICE_DENY_IPV4_METHOD,
		     svc->a.deny.ipv4_reject_method) < 0)
			goto nla_put_failure;
		if (kznl_dump_service_deny_method
		    (skb, KZNL_ATTR_SERVICE_DENY_IPV6_METHOD,
...
static int
kznl_build_service(struct sk_buff *skb, netlink_port_t pid, u_int32_t seq,
		   int flags, const struct kz_service *svc)
{
	struct kz_service_nat_entry *entry;
	...
	if (kznl_build_service_add
	    (skb, pid, seq, flags, KZNL_MSG_ADD_SERVICE, svc) < 0)
		goto nlmsg_failure;
...
kznl_dump_services(struct sk_buff *skb, struct netlink_callback *cb)
{
	...
	/* check if we've finished the dump */
	if (cb->args[SERVICE_DUMP_STATE] ==
	    SERVICE_DUMP_STATE_NO_MORE_WORK)
		return skb->len;

	rcu_read_lock();
	cfg = rcu_dereference(kz_config_rcu);
	if (cb->args[SERVICE_DUMP_STATE] == SERVICE_DUMP_STATE_FIRST_CALL
	    || !kz_generation_valid(cfg,
				    cb->args[SERVICE_DUMP_CONFIG_GEN])) {
		cb->args[SERVICE_DUMP_CONFIG_GEN] = kz_generation_get(cfg);
		cb->args[SERVICE_DUMP_STATE] =
		    SERVICE_DUMP_STATE_HAVE_CONFIG_GEN;
		cb->args[SERVICE_DUMP_CURRENT_SERVICE] = 0;
	}

      restart:
	last =
	    (const struct kz_service *) cb->
	    args[SERVICE_DUMP_CURRENT_SERVICE];
...
kznl_recv_get_service(struct sk_buff *skb, struct genl_info *info)
{
	...
	svc =
	    kz_service_lookup_name(rcu_dereference(kz_config_rcu),
				   service_name);
	if (svc == NULL) {
...
kznl_recv_add_dispatcher(struct sk_buff *skb, struct genl_info *info)
{
	...
	/* fill fields */
	res =
	    kznl_parse_name_alloc(info->attrs[KZNL_ATTR_DISPATCHER_NAME],
				  &dpt->name);
	if (res < 0) {
	...
	res =
	    kznl_parse_dispatcher_n_dimension(info->
					      attrs
	...
	if (p != NULL) {
		kz_err
		    ("dispatcher with the same name already present; name='%s'\n",
		     dpt->name);
		res = -EEXIST;
		goto error_unlock_op;
	}
	...
	res =
	    transaction_add_op(tr, KZNL_OP_DISPATCHER,
			       kz_dispatcher_get(dpt),
			       transaction_destroy_dispatcher);
	if (res < 0) {
		kz_err("failed to queue transaction operation\n");
	}
...
kznl_recv_add_n_dimension_rule(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (!info->attrs[KZNL_ATTR_DISPATCHER_NAME]) {
		kz_err
		    ("required attribtues missing; attr='dispatcher name'\n");
		res = -EINVAL;
		goto error;
	}
	...
	if (!info->attrs[KZNL_ATTR_N_DIMENSION_RULE_SERVICE]) {
		kz_err
		    ("required attribtues missing; attr='service name'\n");
		res = -EINVAL;
		goto error;
	}
	...
	res =
	    kznl_parse_dispatcher_n_dimension_rule(info->
						   attrs
	...
	for (attr_type = KZNL_ATTR_INVALID;
	     attr_type < KZNL_ATTR_TYPE_COUNT; attr_type++) {
		...
		case KZNL_ATTR_TYPE_COUNT:
			kz_err("invalid attribute type; attr_type='%d'",
			       attr_type);
			res = -EINVAL;
			goto error_free_svc_name;
		}
...
static int
kznl_recv_add_n_dimension_rule_entry(struct sk_buff *skb,
				     struct genl_info *info)
{
	int res = 0;
	char *dpt_name = NULL, *src_zone_name = NULL, *dst_zone_name =
	    NULL;
	enum kznl_attr_types attr_type;
	struct kz_dispatcher *dpt;
	struct kz_transaction *tr;
	...
	res =
	    kznl_parse_dispatcher_n_dimension_rule_entry(info->
							 attrs
							 [KZNL_ATTR_N_DIMENSION_RULE_ID],
							 &rule_entry);
	if (res < 0) {
	...
	for (attr_type = KZNL_ATTR_INVALID;
		...
		case KZNL_ATTR_N_DIMENSION_IFACE:{
				res =
				    kznl_parse_name(info->attrs[attr_type],
						    (char *) &rule_entry.
						    ifname,
						    sizeof(rule_entry.
							   ifname));
		...
		case KZNL_ATTR_N_DIMENSION_SRC_PORT:{
				...
				if (res < 0) {
					kz_err
					    ("failed to parse source port range\n");
					goto error_free_names;
				}
		...
		case KZNL_ATTR_N_DIMENSION_SRC_ZONE:{
				...
				if (res < 0) {
					kz_err
					    ("failed to parse source zone name\n");
					goto error_free_names;
				}
		...
		case KZNL_ATTR_N_DIMENSION_DST_IP:{
				...
				if (res < 0) {
					kz_err
					    ("failed to parse destination subnet\n");
					goto error_free_names;
				}
		...
		case KZNL_ATTR_N_DIMENSION_DST_ZONE:{
				...
				if (res < 0) {
					kz_err
					    ("failed to parse destination zone name\n");
					goto error_free_names;
				}
		...
		case KZNL_ATTR_N_DIMENSION_SRC_IP6:{
				...
				if (res < 0) {
					kz_err
					    ("failed to parse source IPv6 subnet\n");
					goto error_free_names;
				}
		...
		case KZNL_ATTR_N_DIMENSION_DST_IP6:{
				...
				if (res < 0) {
					kz_err
					    ("failed to parse destination IPv6 subnet\n");
					goto error_free_names;
				}
	...
	if (src_zone_name != NULL) {
		rule_entry.src_zone =
		    lookup_zone_merged(tr, src_zone_name);
	...
	if (dst_zone_name != NULL) {
		rule_entry.dst_zone =
		    lookup_zone_merged(tr, dst_zone_name);
...
/* !!! must be called with the instance mutex held !!! */
struct kz_bind *kz_bind_lookup_instance(const struct kz_instance *instance,
					const struct kz_bind *bind)
{
	struct kz_bind *i;
...
static inline int
kznl_parse_bind_alloc(struct nlattr *attrs[], unsigned int instance_id,
		      struct kz_instance **instance,
		      struct kz_bind **_bind)
{
	int res = 0;
	struct kz_bind *bind;
	...
	res =
	    kznl_parse_name_alloc(attrs[KZNL_ATTR_INSTANCE_NAME],
				  &instance_name);
	if (res < 0) {
	...
	*instance = kz_instance_lookup(instance_name);
	...
	if ((*instance)->id != instance_id) {
		kz_debug
		    ("transaction instance id and instance id differs; instance_id='%d' tr_instance_id'%d'\n",
	...
	if (kznl_parse_proto(attrs[KZNL_ATTR_BIND_PROTO], &bind->proto) <
	    0) {
		res = -EINVAL;
	...
	if (bind->proto != IPPROTO_TCP && bind->proto != IPPROTO_UDP) {
		kz_err
		    ("only TCP and UDP protocols are supported; proto='%d'\n",
		     bind->proto);
		res = -EINVAL;
		goto error_free_bind;
	}
	...
	if (kznl_parse_inet_addr
	    (attrs[KZNL_ATTR_BIND_ADDR], &bind->addr, &bind->family) < 0) {
		res = -EINVAL;
		goto error_free_bind;
	}
...
static int kznl_recv_add_bind(struct sk_buff *skb, struct genl_info *info)
{
	int res = 0;
	struct kz_transaction *tr;
	struct kz_instance *instance;
	...
	res =
	    kznl_parse_bind_alloc(info->attrs, tr->instance_id, &instance,
				  &bind);
	if (res < 0)
		goto error_unlock_tr;
	...
	if (found_bind
	    && !(found_bind->peer_pid == bind->peer_pid
		 && (tr->flags & KZF_TRANSACTION_FLUSH_BIND))) {
		kz_bind_debug(bind,
			      "bind with the same parameters already present in the instance");
		res = -EEXIST;
		goto error_free_bind;
	}
	...
	if (transaction_bind_lookup(tr, bind)) {
		kz_bind_debug(bind,
			      "bind with the same parameters already present in the transaction");
		res = -EEXIST;
		goto error_free_bind;
	}

	res =
	    transaction_add_op(tr, KZNL_OP_BIND, bind,
			       transaction_destroy_bind);
	if (res < 0) {
		kz_err("failed to queue transaction operation\n");
		goto error_free_bind;
	} else {
		kz_bind_debug(bind,
			      "bind added to transaction operation queue");
	}
...
static int
kznl_dump_bind(struct sk_buff *skb, netlink_port_t pid, u_int32_t seq,
	       int flags, enum kznl_msg_types msg_type,
	       ...
	       const struct kz_bind *bind)
{
	...
	if (kznl_dump_name(skb, KZNL_ATTR_INSTANCE_NAME, instance->name) <
	    0)
		goto nla_put_failure;

	if (kznl_dump_inet_addr
	    (skb, KZNL_ATTR_BIND_ADDR, bind->family, &bind->addr) < 0)
		goto nla_put_failure;
...
static int
kznl_build_instance_bind(struct sk_buff *skb, u_int32_t pid, u_int32_t seq,
			 int flags, const struct kz_instance **instance,
			 const struct kz_bind **bind)
{
	...
	if (!*instance)
		*instance =
		    list_first_entry(&kz_instances, struct kz_instance,
				     list);

	last_bind = *bind;
	if (last_bind) {
		list_for_each_entry((*bind),
				    &(*instance)->bind_lookup->list_bind,
				    list) {
			if (*bind == last_bind)
				break;
		}
	...
	if (!*bind)
		*bind =
		    list_first_entry(&(*instance)->bind_lookup->list_bind,
				     struct kz_bind, list);
	...
	list_for_each_entry_from((*instance), &kz_instances, list) {
		if (*instance != last_instance)
			*bind =
			    list_first_entry(&(*instance)->bind_lookup->
					     list_bind, struct kz_bind,
					     list);

		list_for_each_entry_from((*bind),
					 &(*instance)->bind_lookup->
					 list_bind, list) {
			if (kznl_dump_bind
			    (skb, pid, seq, false, KZNL_MSG_ADD_BIND,
			     *instance, *bind) < 0) {
				goto error;
			}
...
kznl_dump_binds(struct sk_buff *skb, struct netlink_callback *cb)
{
	...
	if (cb->args[BIND_DUMP_ARG_STATE] == BIND_DUMP_STATE_FIRST_CALL ||
	    !kz_generation_valid(config,
				 cb->
				 args[BIND_DUMP_ARG_CONFIG_GENERATION])) {
		cb->args[BIND_DUMP_ARG_INSTANCE] = 0;
		cb->args[BIND_DUMP_ARG_BIND] = 0;
		cb->args[BIND_DUMP_ARG_STATE] =
		    BIND_DUMP_STATE_HAVE_CONFIG_GEN;
		cb->args[BIND_DUMP_ARG_CONFIG_GENERATION] =
		    kz_generation_get(config);
	}

	instance =
	    (const struct kz_instance **) &cb->
	    args[BIND_DUMP_ARG_INSTANCE];
...
static int
kznl_build_dispatcher_add_rule_entry(struct sk_buff *skb, u_int32_t pid,
				     u_int32_t seq, int flags,
				     enum kznl_msg_types msg,
				     const struct kz_dispatcher *const dpt,
				     const struct
				     kz_dispatcher_n_dimension_rule *rule,
				     u_int32_t entry_num)
{
	void *hdr;
...
static int
kznl_build_dispatcher_add_rule(struct sk_buff *skb, u_int32_t pid,
			       u_int32_t seq, int flags,
			       enum kznl_msg_types msg,
			       const struct kz_dispatcher *dpt,
			       const struct kz_dispatcher_n_dimension_rule
			       *rule)
{
	void *hdr;
	...
	if (kznl_dump_name
	    (skb, KZNL_ATTR_N_DIMENSION_RULE_SERVICE,
	     rule->service->name) < 0)
		goto nla_put_failure;
...
static int
kznl_build_dispatcher_add(struct sk_buff *skb, u_int32_t pid,
			  u_int32_t seq, int flags,
			  enum kznl_msg_types msg,
			  const struct kz_dispatcher *dpt)
{
	...
	NLA_PUT(skb, KZNL_ATTR_DISPATCHER_N_DIMENSION_PARAMS,
		sizeof(n_dimension), &n_dimension);
...
static int
kznl_build_dispatcher(struct sk_buff *skb, u_int32_t pid, u_int32_t seq,
		      int flags, const struct kz_dispatcher *dpt,
		      long *part_idx, long *rule_entry_idx)
{
	...
	if (*part_idx == 0) {
		...
		if (kznl_build_dispatcher_add
		    (skb, pid, seq, flags, KZNL_MSG_ADD_DISPATCHER,
		     dpt) < 0)
			goto nlmsg_failure;
		*part_idx = 1;
	}
	...
	for (; (*part_idx) <= (long) dpt->num_rule; ++(*part_idx)) {
		...
		const struct kz_dispatcher_n_dimension_rule *rule =
		    &dpt->rule[(*part_idx) - 1];
		kz_debug("part_idx=%ld, rule_entry_idx=%ld", *part_idx,
			 *rule_entry_idx);

		if (*rule_entry_idx == 0) {
			msg_rollback = skb_tail_pointer(skb);
			if (kznl_build_dispatcher_add_rule
			    (skb, pid, seq, flags, KZNL_MSG_ADD_RULE, dpt,
			     rule) < 0)
				goto nlmsg_failure;
			*rule_entry_idx = 1;
		}
...
#undef UPDATE_MAX_ENTRY_NUM
		...
		for (; (*rule_entry_idx) <= max_entry_num;
		     ++(*rule_entry_idx)) {
			kz_debug("rule_entry_idx=%ld", *rule_entry_idx);
			msg_rollback = skb_tail_pointer(skb);
			if (kznl_build_dispatcher_add_rule_entry
			    (skb, pid, seq, flags, KZNL_MSG_ADD_RULE_ENTRY,
...
*/
	...
	/* check if we've finished the dump */
	if (cb->args[DISPATCHER_DUMP_ARG_STATE] ==
	    DISPATCHER_DUMP_STATE_NO_MORE_WORK)
		return skb->len;
	...
	if (cb->args[DISPATCHER_DUMP_ARG_STATE] ==
	    ...
	    || !kz_generation_valid(cfg,
				    ...
				    [DISPATCHER_DUMP_ARG_CONFIG_GENERATION]))
	{
		cb->args[DISPATCHER_DUMP_ARG_CONFIG_GENERATION] =
		    kz_generation_get(cfg);
		cb->args[DISPATCHER_DUMP_ARG_STATE] =
		    DISPATCHER_DUMP_STATE_HAVE_CONFIG;
      ...
      restart:
	last =
	    (const struct kz_dispatcher *) cb->
	    args[DISPATCHER_DUMP_ARG_CURRENT_DISPATCHER];
...
kznl_recv_get_dispatcher(struct sk_buff *skb, struct genl_info *info)
{
	...
	dpt =
	    kz_dispatcher_lookup_name(rcu_dereference(kz_config_rcu),
				      dpt_name);
	if (dpt == NULL) {
	 ...
	 */
	do {
		kz_debug("dpt_item_idx=%ld, rule_entry_idx=%ld",
			 dpt_item_idx, rule_entry_idx);
		/* create skb and dump */
		nskb = genlmsg_new(NLMSG_GOODSIZE, GFP_KERNEL);
		if (!nskb) {
			kz_err("failed to allocate reply message\n");
...
static int
kznl_build_query_resp(struct sk_buff *skb, u_int32_t pid, u_int32_t seq,
		      int flags, enum kznl_msg_types msg,
		      struct kz_dispatcher *dispatcher,
		      ...
		      struct kz_service *service)
{
	...
	if (dispatcher
	    && kznl_dump_name(skb, KZNL_ATTR_DISPATCHER_NAME,
			      dispatcher->name) < 0)
		goto nfattr_failure;
	if (client_zone
	    && kznl_dump_name(skb, KZNL_ATTR_QUERY_REPLY_CLIENT_ZONE,
			      client_zone->name) < 0)
		goto nfattr_failure;
	if (server_zone
	    && kznl_dump_name(skb, KZNL_ATTR_QUERY_REPLY_SERVER_ZONE,
			      server_zone->name) < 0)
		goto nfattr_failure;
	if (service
	    && kznl_dump_name(skb, KZNL_ATTR_SERVICE_NAME,
			      service->name) < 0)
		goto nfattr_failure;
...
static int kznl_recv_query(struct sk_buff *skb, struct genl_info *info)
{
	int res = 0;
	struct kz_query query;
	...
	/* fill fields */
	res =
	    kznl_parse_inet_addr(info->
				 attrs[KZNL_ATTR_QUERY_PARAMS_SRC_IP],
				 &query.src_addr, &query.src_addr_family);
	if (res < 0) {
	...
	res =
	    kznl_parse_inet_addr(info->
				 attrs[KZNL_ATTR_QUERY_PARAMS_DST_IP],
				 &query.dst_addr, &query.dst_addr_family);
	if (res < 0) {
		kz_err("failed to parse src ip nested attribute\n");
		goto error;
	}
	...
	res =
	    kznl_parse_query_params(info->attrs[KZNL_ATTR_QUERY_PARAMS],
				    &query);
	if (res < 0) {
	...
	if (info->attrs[KZNL_ATTR_QUERY_PARAMS_REQID]) {
		...
		res =
		    kznl_parse_reqid(info->
				     attrs[KZNL_ATTR_QUERY_PARAMS_REQID],
				     &query.reqids.vec[0]);
		if (res < 0) {
	...
	kz_lookup_session(rcu_dereference(kz_config_rcu), &query.reqids,
			  dev, query.src_addr_family, &query.src_addr,
			  &query.dst_addr, query.proto, query.src_port,
			  query.dst_port, &dispatcher, &client_zone,
			  &server_zone, &service, 0);
...
static int
kznl_build_get_version_resp(struct sk_buff *skb, u_int32_t pid,
			    u_int32_t seq, int flags,
			    enum kznl_msg_types msg)
{
	void *hdr;
...
kznl_recv_get_version(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (kznl_build_get_version_resp
	    (nskb, info->snd_pid, info->snd_seq, 0,
...
kznl_netlink_event(struct notifier_block *n, unsigned long event, void *v)
{
	...
	if (event == NETLINK_URELEASE &&
	    notify->protocol == NETLINK_GENERIC && notify->pid != 0) {
		...
		list_for_each_entry(instance, &kz_instances, list) {
			if (instance->id == 0) {
				kz_debug
				    ("no cleanup for global instance\n");
			} else {
				kz_debug("cleaning up instance; id='%d'\n",
					 instance->id);
			}
			kz_instance_remove_bind(instance, notify->pid,
						NULL);
		}
...
int __init kz_netlink_init(void)
{
	...
	res =
	    genl_register_family_with_ops(&kznl_family, kznl_ops,
					  ARRAY_SIZE(kznl_ops));
	if (res < 0) {
