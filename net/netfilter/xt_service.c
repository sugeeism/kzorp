...
service_mt(const struct sk_buff *skb, struct xt_action_param *par)
{
	struct ipt_service_info *info =
	    (struct ipt_service_info *) par->matchinfo;
	...
	if (kzorp == NULL) {
		kz_debug
		    ("cannot add kzorp extension, doing local lookup\n");
		kzorp = &local_kzorp;
...
static int service_mt_checkentry(const struct xt_mtchk_param *par)
{
	struct ipt_service_info *info =
	    (struct ipt_service_info *) par->matchinfo;
...
{
	return xt_register_matches(service_match,
				   ARRAY_SIZE(service_match));
}
