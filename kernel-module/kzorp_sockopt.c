...
#include "include/kzorp_sockopt.h"
#include <net/netfilter/nf_conntrack.h>
...
static int
kzorp_getsockopt_results(u8 family, struct sock *sk, int optval,
			 void __user * user, int *len)
{
	...
	case PF_INET:
		kz_debug
		    ("getting results; proto='%u', src='%pI4:%hu', dst='%pI4:%hu'\n",
		     sk->sk_protocol, &inet_sk(sk)->inet_rcv_saddr,
	...
	case PF_INET6:
		kz_debug
		    ("getting results; proto='%u', src='%pI6:%hu', dst='%pI6:%hu'\n",
		     sk->sk_protocol, &inet6_sk(sk)->rcv_saddr,
		     ntohs(inet_sk(sk)->inet_sport), &inet6_sk(sk)->daddr,
		     ntohs(inet_sk(sk)->inet_dport));
		break;
	default:
		BUG();
	...
	if ((unsigned int) *len < sizeof(struct kz_lookup_result)) {
		kz_debug
		    ("buffer size is too small for the result; len='%d', required='%lu'\n",
	...
	case PF_INET:
		...
		break;
	case PF_INET6:
		ipv6_addr_copy(&tuple.src.u3.in6,
	...
	if (h) {
		...
		rcu_read_lock();
		{
			...
			const struct kz_config *cfg =
			    rcu_dereference(kz_config_rcu);
			cookie =
			    kz_generation_valid(cfg,
						kzorp->generation) ? cfg->
			    cookie : 0;
		}
		rcu_read_unlock();

		kz_debug
		    ("found kzorp results; client_zone='%s', server_zone='%s', dispatcher='%s', service='%s'\n",
		     kzorp->czone ? kzorp->czone->
		     unique_name : kz_log_null,
		     kzorp->szone ? kzorp->szone->
		     unique_name : kz_log_null,
		...
		if (kzorp->czone)
			COPY_NAME_TO_USER(user, czone_name,
					  kzorp->czone->unique_name);
		if (kzorp->szone)
			COPY_NAME_TO_USER(user, szone_name,
					  kzorp->szone->unique_name);
		if (kzorp->dpt)
			COPY_NAME_TO_USER(user, dispatcher_name,
					  kzorp->dpt->name);
		if (kzorp->svc)
			COPY_NAME_TO_USER(user, service_name,
					  kzorp->svc->name);
...
static int
kzorp_getsockopt_results_v4(struct sock *sk, int optval,
			    void __user * user, int *len)
{
...
static int
kzorp_getsockopt_results_v6(struct sock *sk, int optval,
			    void __user * user, int *len)
{
...
int __init kz_sockopt_init(void)
{
	int res;
